...
$ajax_url = html_entity_decode(wp_nonce_url('admin-ajax.php?action=exitpopup_import_settings','exitpopup_import_settings'));
...
echo "<script>
      jQuery(document).ready(function($) {
        $('#import-settings-btn').click(function() {
            if(confirm(exitpopup_msgs.import_confirm)){
                ...
                $.post('{$ajax_url}',{settings: settings}, function(data) {
                	if(data == '1'){
                    	...
                    	setTimeout('exitpopup_reload_page()',2000);
                  	}
                });
            }
        }); 
