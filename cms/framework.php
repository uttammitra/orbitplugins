...
class exitpopup_ADMIN
{
    public $plugin_version = exitpopup_VERSION;
    public $plugin_name = exitpopup_PLUGIN_NAME;
   ...
   /**
    ...
    {
        if ( isset( $_POST[ 'exitpopup_reset' ] ) ) {
            $option_page = $_POST[ 'option_page' ];
            check_admin_referer( $option_page . '-options' );
            require_once(exitpopup_PLUGIN_PATH.'include/default-settings.php');

            $_POST[ $_POST[ 'option_page' ] ] = $exitpopup_settings_deafults[$_POST[ 'option_page' ]];
            add_settings_error( 'general', 'exitpopup-settings-reset', __( "Settings reset." ), 'updated' );
        }
    }
    ...
    function admin_enqueue_scripts( $hook_suffix )
    {
        ...
        wp_enqueue_script( 'exitpopup-framework-js', exitpopup_PLUGIN_URL . 'cms/settings-scripts.js', array( 'jquery' ), $this->plugin_version );
        wp_enqueue_script( 'theme-preview' );
        wp_enqueue_style( 'thickbox' );
        wp_enqueue_style( 'media-upload' );
        wp_enqueue_style( 'wp-color-picker' );
        wp_enqueue_style( 'exitpopup-framework-css', exitpopup_PLUGIN_URL . 'cms/settings-style.css', false, $this->plugin_version );
    ...
    {
        $plugin_file = exitpopup_SLUG;

        if ( $file == $plugin_file ) {
            $settings_link = '<a href="options-general.php?page=exitpopup">Settings</a>';
    ...
    function plugin_options_tabs( )
    {
        ...
        foreach ( exitpopup_get_options() as $v ) {
            if ( $v[ 'type' ] == 'tab' ) {
                $uses_tabs = true;
                break;
            }
        ...
        if ( $uses_tabs ) {
            ...
            echo '<a class="nav-tab exitpopup-preview thickbox-preview" href="'.home_url().'?cs_preview=true&TB_iframe=true&width=640&height=632" title="'.__('&larr; Close Window','exitpopup').'">'.__('Live Preview','exitpopup').'</a>';            
            echo '</h2>';
    ...
    function option_page( )
    {
        ...
        <?php screen_icon(); ?>
            <h2><?php echo $this->plugin_name; ?> <span class="exitpopup-version"> <?php echo exitpopup_VERSION; ?></span></h2>
            <?php //settings_errors() ?>
            ...
            <?php endif; ?>
                    <?php if(!empty($_GET['tab']))
                            do_action( 'exitpopup_render_page', array('tab'=>$_GET['tab']));
                    ?>
                    <form action="options.php" method="post">

                    <!-- <input name="submit" type="submit" value="<?php _e( 'Save All Changes', 'exitpopup' ); ?>" class="button-primary"/> -->
                    <?php if(!empty($_GET['tab']) && $_GET['tab'] != 'exitpopup_tab_3') { ?>
                    <!-- <input id="reset" name="reset" type="submit" value="<?php _e( 'Reset Settings', 'exitpopup' ); ?>" class="button-secondary"/>     -->
                    <?php } ?>
                                        ...
                                        switch ( $v[ 'type' ] ) {
                                            ...
                                            case 'section':
                                                ...
                                                if ( $current_tab == $tab[ 'id' ] or $current_tab === false ) {
                                                    if ( $layout == '2-col' ) {
                                                        echo '<div id="'.$v[ 'id' ].'" class="postbox exitpopup-postbox">';
        ...
        <script>
            jQuery(document).ready(function($) {
                $('#reset').click(function(e){
                    if(!confirm('<?php _e( 'This tabs settings be deleted and reset to the defaults. Are you sure you want to reset?', 'exitpopup' ); ?>')){
                        e.preventDefault();
                    }
                });
            });
    ...
    {
        foreach ( exitpopup_get_options() as $k => $v ) {
    ...
    function field_machine( $args )
    {
        ...
        // Load defaults
        $defaults = array( );
        foreach ( exitpopup_get_options() as $k ) {
            switch ( $k[ 'type' ] ) {
                case 'setting':
                case 'section':
                case 'tab':
        ...
        $path = exitpopup_PLUGIN_PATH . 'cms/field-types/' . $type . '.php';
        if ( file_exists( $path ) ) {
            // Show Field
            include( $path );
            // Show description
                ...
                default:
                    if ( !empty( $k[ 'validate' ] ) && $setting_id == $option_page ) {
                        ...
                        foreach ( $validation_rules as $v ) {
                            $path = exitpopup_PLUGIN_PATH . 'cms/validations/' . $v . '.php';
                            if ( file_exists( $path ) ) {
                                // Defaults Values
                                $is_valid  = true;
                                $error_msg = '';
                                ...
                                // Is it valid?
                                if ( $is_valid === false ) {
                                    add_settings_error( $k[ 'id' ], 'exitpopup_error', $error_msg, 'error' );
                                    // Unset invalids
                                    unset( $input[ $k[ 'id' ] ] );
                                }
    ...
    /**
     * exitpopup version of WP's do_settings_sections
     *
     * @since 0.1.0
     */
